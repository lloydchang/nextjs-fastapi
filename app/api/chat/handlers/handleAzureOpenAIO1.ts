...
export async function handleTextWithAzureOpenAIO1Model(input: HandlerInput, config: any): Promise<string> {
  ...
  try {
    logger.info(`app/api/chat/handlers/handleAzureOpenAIO1.ts - Handling text with AzureOpenAIO1. Prompt: ${prompt}, Model: ${model}`);
    const response = await serveAzureOpenAIO1(prompt, model, config);
    logger.info(`app/api/chat/handlers/handleAzureOpenAIO1.ts - AzureOpenAIO1 response: ${response}`);
    return response;
  } catch (error: any) {
    logger.error(`app/api/chat/handlers/handleAzureOpenAIO1.ts - Error: ${error.message}`);
    throw error;
  }
}

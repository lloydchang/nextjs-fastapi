 ...
 */
export async function generateFromCloudflareGemma(params: {
  endpoint: string;
  prompt: string;
  model: string;
  token: string;
  ...
  if (!xAuthKey) {
    logger.error('app/api/chat/clients/CloudflareGemmaClient.ts - Cloudflare Gemma API key (xAuthKey) is not defined.');
  ...
  try {
    ...
    // logger.silly(`app/api/chat/clients/CloudflareGemmaClient.ts - Request Headers Sent: ${JSON.stringify(maskedHeaders, null, 2)}`);
    // logger.silly(`app/api/chat/clients/CloudflareGemmaClient.ts - Request Body Sent: ${requestBody}`);
    ...
    // logger.silly(`app/api/chat/clients/CloudflareGemmaClient.ts - cURL Equivalent:\n\n${curlCommand}`);
    ...
    // logger.silly(`app/api/chat/clients/CloudflareGemmaClient.ts - Response Headers Received: ${JSON.stringify(responseHeaders, null, 2)}`);

    if (!response.ok) {
      logger.error(`app/api/chat/clients/CloudflareGemmaClient.ts - HTTP error! Status: ${response.status}`);
      return null;
    }

    const reader = response.body?.getReader();
    if (!reader) {
      logger.error('app/api/chat/clients/CloudflareGemmaClient.ts - Failed to access the response body stream.');
      return null;
    }
    ...
    // logger.silly(`app/api/chat/clients/CloudflareGemmaClient.ts - Response Body Received: ${finalResponse}`);

    return finalResponse;
  } catch (error) {
    logger.warn(`app/api/chat/clients/CloudflareGemmaClient.ts - Error generating content from Cloudflare Gemma: ${error}`);
    return null;
  }
}

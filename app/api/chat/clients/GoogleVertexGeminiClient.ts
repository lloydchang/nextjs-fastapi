...
export async function generateFromGoogleVertexGemini({
  ...
  if (!isValid) {
    if (!hasWarnedGoogleVertexGemini) {
      logger.warn(`app/api/chat/clients/GoogleVertexGeminiClient.ts - Optional environment variables are missing or contain invalid placeholders: ${optionalVars.join(', ')}`);
  ...
  logger.info(`app/api/chat/clients/GoogleVertexGeminiClient.ts - Sending request to Google Vertex Gemini. Model: ${model}, Prompt: ${combinedPrompt}`);
  ...
  try {
    ...
    logger.info(`app/api/chat/clients/GoogleVertexGeminiClient.ts - Generated text from Google Vertex Gemini: ${generatedText}`);
    return generatedText;
  } catch (error) {
    logger.error(`app/api/chat/clients/GoogleVertexGeminiClient.ts - Error generating content from Google Vertex Gemini: ${error}`);
    return null;
  }
}

...
export function RateLimitController(ip: string) {
  ...
  if (record) {
    if (currentTime - record.startTime < WINDOW_SIZE) {
      if (record.count >= RATE_LIMIT) {
        logger.warn(`app/api/chat/controllers/RateLimitController.ts - Rate limit exceeded for IP: ${ip}`);
  ...
  logger.info(`app/api/chat/controllers/RateLimitController.ts - IP: ${ip} request count: ${record?.count || 1}`);
  return null;
}

...
export async function handleTextWithOpenAIO1Model({ prompt, model }: { prompt: string; model: string }, config: any): Promise<string> {
  const { OPENAI_O1_ENDPOINT } = process.env;

  if (!OPENAI_O1_ENDPOINT || !model) {
    throw new Error('OpenAI O1: Required environment variables are missing.');
  }

  const payload = { model, prompt };
  logger.debug(`app/api/chat/controllers/OpenAIO1Controller.ts - Sending payload: ${JSON.stringify(payload)}`);

  const response = await fetch(OPENAI_O1_ENDPOINT, {
    method: 'POST',
  ...
  if (!response.ok) {
    throw new Error(`OpenAI O1: HTTP error! status: ${response.status}`);
  }
